...
func TestSinkWriteAllSupportedFieldTypes(t *testing.T) {
	...
	sink.Write(context.Background(), hlog.LevelInfo, "typed", map[string]any{
		"s":   "x",
		"i":   int(1),
		"i8":  int8(2),
		"i16": int16(3),
...
func TestSinkWriteMapsAllLevelsAndMessageBehavior(t *testing.T) {
	tests := []struct {
		...
		{name: "debug", level: hlog.LevelDebug, message: "m", wantLevel: "debug", wantMessage: "m"},
		{name: "warn", level: hlog.LevelWarn, message: "m", wantLevel: "warn", wantMessage: "m"},
		{name: "error", level: hlog.LevelError, message: "m", wantLevel: "error", wantMessage: "m"},
