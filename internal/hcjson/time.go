...
// true (a float otherwise) and appends the encoded value to dst, so
// duration fields render like the zerolog adapter (float milliseconds
// with shortest round-trip precision by default).
...
//
// The formatted string is cached per Unix second, so the common case is
// an atomic pointer load. The cache assumes all callers format times in
// the process's local zone (the JSON sink always passes time.Now);
// formatting times from other locations may reuse the wrong zone
// rendering for the shared second — use AppendTime for arbitrary
// locations. Concurrent use is safe: stores race benignly and the
// rendered value for a given second is deterministic.
